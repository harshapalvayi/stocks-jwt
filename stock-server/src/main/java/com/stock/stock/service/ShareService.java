            ...
            for (Share share : shares) {
                ...
                if (yahooStock.isValid()) {
                    ...
                    if (equity != null && investment != null) {
                        equity = equity.add(price.multiply(BigDecimal.valueOf(share.getShares())));
                        investment = investment.add(buy.multiply(BigDecimal.valueOf(share.getShares())));
                        percentChange = ((((investment.subtract(equity))
                                        .multiply(BigDecimal.valueOf(100)))
                                        .divide(equity, RoundingMode.FLOOR))
                                        .setScale(2, RoundingMode.FLOOR)).abs() + "%";
                    }
                }
            }
        }
        return  new PortfolioDto(investment, equity, annualDividend, percentChange);
    }
